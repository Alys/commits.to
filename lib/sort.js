...
// Given time t and deadline d (both unixtime in seconds), return the absolute 
// distance in seconds between t & the deadline's Schelling fence nearest to t.
const scheldist = (t, d) => {
  return Math.min(
    Math.abs(t - d),         // abs difference between t  &  deadline
    ...
    Math.abs(t - d - SIY),   // abs difference between t  &  1 year late
  )
}
...
//   (if no tdue, no tfin, and tini in the future then it could sort after 
